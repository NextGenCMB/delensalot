...
import dlensalot.iterators.cs_iterator
from dlensalot.utils import cli, read_map
from dlensalot.utils_hp import Alm, almxfl, alm2cl
from dlensalot import utils_qe
from dlensalot.utils_scarf import scarfjob, pbdGeometry, pbounds
from dlensalot import utils_dlm

from plancklens.qcinv import multigrid
from dlensalot.iterators import bfgs, steps

from dlensalot.opfilt import opfilt_base
...
class iterator_cstmf(dlensalot.iterators.cs_iterator.qlm_iterator):
    """Constant mean-field
    ...
    def calc_gradlik(self, itr, key, iwantit=False):
        ...
        if not self._is_qd_grad_done(itr, key) or iwantit:
            ...
            if PorT: # Pol rec.
                delE = almxfl(self.dat_maps[0], cli(self.filter.transf_elm), mmax, False)
                delB = almxfl(self.dat_maps[1], cli(self.filter.transf_blm), mmax, False)
                delE, delB = ffi.lensgclm([delE, delB], self.filter.mmax_len, 2, self.filter.lmax_len, self.filter.mmax_len, backwards=True, nomagn=True)
                almxfl(delE, self.filter.transf_elm, mmax, True)
                almxfl(delB, self.filter.transf_blm, mmax, True)
            else: # TT-rec
            ...
            if it_soltn < itr - 1:
                ...
                if PorT:
                    mchain.solve(soltn, np.array([delE, delB]), dot_op=self.filter.dot_op())
                else:
