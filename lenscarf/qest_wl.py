...
def get_qlms_wl(qudat:np.ndarray or list, elm_wf:np.ndarray, filt:opfilt_ee_wl.alm_filter_ninv_wl):
    ...
    repmap, impmap = get_irespmap(qudat, ebwf, filt)
    Gs, Cs = get_gpmap(ebwf, 3, filt)  # 2 pos.space maps
    GC = (repmap - 1j * impmap) * (Gs + 1j * Cs)  # (-2 , +3)
    Gs, Cs = get_gpmap(ebwf, 1, filt)
    GC -= (repmap + 1j * impmap) * (Gs - 1j * Cs)  # (+2 , -1)
    del repmap, impmap, Gs, Cs
    lmax_qlm = filt.ffi.lmax_dlm
    mmax_qlm = filt.ffi.mmax_dlm
    G, C = filt.ffi.geom.map2alm_spin([GC.real, GC.imag], 1, lmax_qlm, mmax_qlm, filt.ffi.sht_tr, (-1., 1.))
    del GC
...
def get_irespmap(qudat:np.ndarray, ebwf:np.ndarray or list, filt:opfilt_ee_wl.alm_filter_ninv_wl):
    ...
    return filt.ffi.geom.alm2map_spin(ebwf, 2, filt.lmax_len, filt.mmax_len, filt.ffi.sht_tr, (-1., 1.))

def get_gpmap(eblm_wf:np.ndarray or list, spin:int, filt:opfilt_ee_wl.alm_filter_ninv_wl):
    ...
    return filt.ffi.gclm2lenmap(eblm, filt.mmax_sol, spin, False)
