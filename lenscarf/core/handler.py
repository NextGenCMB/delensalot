    ...
    def run(self):
        for idx in self.jobs[mpi.rank::mpi.size]:
            ...
            if self.run_config.itmax >= 0 and Rec.maxiterdone(lib_dir_iterator) < self.run_config.itmax:
                itlib = self.ith(self.run_config.k, idx, self.run_config.v, self.libdir_iterators, self.lensing_config, self.survey_config, self.run_config)
                ...
                for i in range(self.run_config.itmax + 1):
                    ...
                    itlib_iterator.chain_descr  = self.lensing_config.chain_descr(self.lensing_config.lmax_unl, self.run_config.tol_iter(i))
                    itlib_iterator.soltn_cond   = self.run_config.soltn_cond(i)
