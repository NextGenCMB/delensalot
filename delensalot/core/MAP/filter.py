...
class WF:
    ...
    def calc_prep(self, qumap):
        ...
        if qumap.dtype in [np.complex64, np.complex128]:
            eblmc = self.inv_operator.act(eblm, adjoint=False)
        else:
            eblm = self.inv_operator.apply_map(qumap)
        eblmc = self.beam_operator.act(eblm, adjoint=False)
    ...
    def precon_op(self, elm):
        ...
        # ninv_fel = _extend_cl(self.beam_operator.transferfunction['e']*2, len(self.inoise_operator.n1eblm[0])-1) * self.inoise_operator.n1eblm[0] # TODO NOISE inject get_febl() here
...
class WF_isotropic:
    ...
    def fwd_op(self, ewflm):
        ...
        eblm = self.inoise_operator.act(eblm, adjoint=False) # TODO NOISE inject apply_map here, use synthesis before
    ...
    def precon_op(self, elm):
        ...
        assert len(self.cls_filt['ee']) > Alm.getlmax(elm.size, None), (Alm.getlmax(elm[0].size, None), len(self.cls_filt['ee']))
        # FIXME indexing must change for MV and T-only
        ninv_fel, ninv_fbl = self.inv_operator.get_febl()
        # Extend transfer function to avoid preconditioning with zero (~ Gaussian beam)
        if len(ninv_fel) - 1 < Alm.getlmax(elm.size, None):
            assert np.all(ninv_fel >= 0)
            nz = np.where(ninv_fel > 0)
    ...
    def get_template(self, it, secondary, component, lm_max_in=None, lm_max_out=None):
        ...
        #     lm_max_in_prev, lm_max_out_prev = self.wf_operator.update_lm_max(lm_max_in=lm_max_in, lm_max_out=lm_max_out)
        #     ret = self.wf_operator.act(estCMB, secondary=secondary)
        #     self.wf_operator.update_lm_max(lm_max_in=lm_max_in_prev, lm_max_out=lm_max_out_prev)
        #     return ret
        # else:
        return self.wf_operator.act(estCMB, secondary=secondary)
